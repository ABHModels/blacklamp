...
double eps3 = 0., a13 = 0., a22 = 0., a52 = 0., g[4][4], dg_dr[4][4];
int imax = 100;
...
int main(int argc, char* argv[])
{
    ...
    snprintf(filename, sizeof(filename), "data/lp_%f_%f_%f.dat", a, height, Mdot);
    printf("%s  \n", filename);
    ...
    johannsen_isco = kerr_rms(chi);//find_isco();
    ...
    starting_r = 0.999 * johannsen_isco;
    eta = 1.0 - specific_energy(johannsen_isco);
    // printf(" isco1=%f, kerr=%f\n", johannsen_isco, kerr_rms(chi));


    for (i = 0; i < imax - 1; i++) {
        rDisk[i] = pow((double)i/(imax-2),3) * (1000. - johannsen_isco) + johannsen_isco;
        RayCount[i] = 0;
    }
    rDisk[imax-1] = 1050;
    ...
    for (i = 0; delta < Emis_delta[1]; i++, delta += stepDelta) {
        ...
        // printf(" rho=%f, z=%f\n", rho, z);
        if(rho > johannsen_isco)
            disk_height = 3.0*Mdot*(1.0 - sqrt(johannsen_isco/rho))/eta;
        else
            disk_height = 0.0;


        for(; z > disk_height && vars[0] <= d && vars[0] > 1.0 + sqrt(1.0 - a*a) + 1.0e-4; ) {
            cache();
            rk();
