...
int main(int argc, char* argv[])
{
    ...
    //hx_start = 0.2; //inner radius of disk
    //hx_end = 1.0; // outer radius of the disk (r)
    //hy_start = 4.52267; //atof(argv[3]); //height of the disk (h)
    
    double rh;
    rh = 1.0 + sqrt(1.0 - a*a);
    int h_index;
    
    for(hx_end = 1.0; hx_end<=25.; hx_end += 25./15.){
        ...
        sprintf(filename,"a%.05e.h_%.03e_r_%.03e_e_%.03e.a13_%.03e.a22_%.03e.a52_%.03e.dat",a,hy_start,hx_end,eps3,a13,a22,a52);
        
        out = fopen(filename,"w");
        
        for(hx_start = 0.5; hx_start<=hx_end; hx_start += 2/30.){ // radial distribution of emitting points on the coronal disk
            ...
            printf("hx_start %f, hx_end %f ,hy_start %f, rr_start %f, theta_start %f\n", hx_start, hx_end ,hy_start, rr_start, theta_start);
            
            
            
            for(beta = 0.0000001; beta<6.2831853; beta += 6.2831853/14.){ // the loop on the beta
                
                double cos_alpha;
                
                for(cos_alpha = -1; cos_alpha< 1.; cos_alpha += 1./11999.){ // loop on delta
                    ...
                    omega_cr_term = 0.0; //pow(hx_start, 1.5);
                    ...
                    //printf("mult %f\n",denom_fact1_e_phit*denom_fact2_e_phit*denom_fact3_e_phit);
                    
                    denom_e_phit = sqrt(1 - (denom_fact2_e_phit*denom_fact1_e_phit));
                    
                    //printf("denom_fact1_e_phit %f, denom_fact2_e_phit %f, denom_fact3_e_phit %f\n", denom_fact1_e_phit, denom_fact2_e_phit, denom_fact3_e_phit);
                    ...
                    //printf("r = %f, theta =  %f, phi = %f,  kr = %f, kth = %f, kphi %f, omega_cr %f ,E %f, unorm %f\n", vars[0], vars[1] , vars[4], vars[2], vars[3], kphi, omega_cr ,E, unorm);
                    //printf(" unorm=%f, E=%f\n", unorm, E);
                    
                    // kt = sqrt(vars[2]*vars[2] + r2*vars[3]*vars[3]);
                    
                    vars[2] /= E;
                    vars[3] /= E;
                    
                    //printf("sigma %f, omegar_cr_term %f, omega_cr %f, omega_small %f, numeretor_e_phit %f, denom_fact1_e_phit %f, denom_fact2_e_phit %f, denom_e_phit %f, e_phit %f, kphi %f, E %f, unorm %f, vars2 %f, vars3 %f \n",sigma, omega_cr_term, omega_cr, omega_small, numeretor_e_phit, denom_fact1_e_phit, denom_fact2_e_phit, denom_e_phit, e_phit, kphi, E, unorm, vars[2], vars[3] );
                    ...
                    //printf("r = %f, theta =  %f, phi = %f,  kr = %f, kth = %f\n", vars[0], vars[1] , vars[4], vars[2], vars[3]);
                    ...
                    for(k = 0; !horizon && z*oldz >= 0. && vars[0] <= d && vars[0] > 1.0 + sqrt(1.0 - a*a) + 0.001; k++) {
                        ...
                        // printf(" %f %f %f \n", vars[0], vars[1], vars[0]*cos(vars[1]));
                        
                        //printf("r = %f, theta =  %f, phi = %f,  kr = %f, kth = %f, kphi %f\n", vars[0], vars[1] , vars[4], vars[2], vars[3], kphi);
                    ...
                    if (z * oldz <= 0. && vars[0] >= kerr_starting_r*0.9 && vars[0] <= 1000.) {
                        ...
                        //g_lp_array[i] = g_lp; //red-shift factor storing in an arrya
    ...
    }  // closing the loop over hx_end (r) i.e the radius of the disk
    return 0;
